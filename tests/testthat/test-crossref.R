...
test_that(
  "get_crossref_articles() returns expected structure for a query", {
  ...
  expect_setequal(names(processed_results), c(
    ...
    ".url", ".references", ".citations", ".related", ".ids"
  ))

  # Check if the publication year is 2020 for non-NA values
  # Rather than checking actual values, we'll just check if the year column exists
  expect_true(".year" %in% names(processed_results))
  ...
  if (length(non_na_years) > 0) {
    expect_true(all(non_na_years == "2020"))
  }

  # Check .ids is a list column with data frames
  expect_true(is.list(processed_results$.ids))
  ...
  expect_true("DOI" %in% names(first_ids))
})
...
test_that(
  "get_crossref_linked() handles invalid or empty ID gracefully", {
...
test_that("get_crossref_linked() works with multiple DOIs", {
  skip_on_cran()
  skip("Skipping multiple DOI test to avoid excessive API calls")
  ...
  if (nrow(linked_dois$references) > 0) {
    expect_true(all(linked_dois$references$source_id %in% dois))
  }
})
...
test_that("cr_prepare_ids handles various ID formats", {
  ...
  # Test multiple DOIs
  expect_equal(
    cr_prepare_ids(c("10.1371/journal.pone.0266781", "10.1037/0003-066X.59.1.29")),
