...
#'
#' @export
get_openalex_articles_new <- function(
  ids = NULL,
  query = NULL,
  year_filter = NULL,
  filters = NULL,
  ...
  if (!is.null(year_filter)) {
    year_params <- oa_parse_year_filter_new(year_filter)
  ...
  if (!is.null(ids)) {
    ...
    results <- oa_fetch_by_ids_new(ids, select_param, filters, per_page, max_results)
  } else {
    # Handle search-based requests
    results <- oa_fetch_by_query_new(query, select_param, filters, per_page, max_results)
  }
  ...
  output <- oa_process_response_new(results)
  if (getOption("bibliobutler.dev_mode", FALSE)) {
...
#'   `get_openalex_articles_new()` for details on supported ID formats.
...
#' get_openalex_linked_new("10.1371/journal.pone.0266781")
#' 
#' # Get only citations and references for multiple articles
#' get_openalex_linked_new(
#'   c("10.1371/journal.pone.0266781", "10.1371/journal.pone.0267149"),
...
#' }
#' @export
get_openalex_linked_new <- function(
  ids,
  ...
  works_df <- get_openalex_articles_new(
    ids = ids,
  ...
  if ("citations" %in% links) {
    ...
    citations_df <- oa_fetch_by_query_new(
      query = NULL,
    ...
    if (nrow(citations_df) > 0) {
      citations_processed <- oa_process_response_new(citations_df)
...
#'
#' @param ids Character vector of prepared IDs (output from oa_prepare_ids_new)
...
#'
#' @keywords internal
oa_fetch_by_ids_new <- function(ids, select_param, filters, per_page, max_results) {
  # Prepare and validate IDs
  prepared_ids <- oa_prepare_ids_new(ids)
  ...
  if (all(purrr::map_int(ids_by_type, ~ length(.x)) <= 50)) {
    ...
    resp <- oa_make_api_call_new("https://api.openalex.org/works", "GET", query = query_params)
  ...
  reqs <- purrr::map(batch_list, ~ oa_make_api_call_new(
...
#'
#' @keywords internal
oa_fetch_by_query_new <- function(query, select_param, filters, per_page, max_results) {
  ...
  count_resp <- oa_make_api_call_new(
    "https://api.openalex.org/works",
    "GET",
    query = count_params
  )
  ...
  if (pages_needed <= 3) {
    ...
    for (page in seq_len(pages_needed)) {
      ...
      resp_data <- oa_make_api_call_new(
        "https://api.openalex.org/works",
        "GET",
        query = query_params
      )
...
#'   
#' @keywords internal
oa_make_api_call_new <- function(url, method = "GET", query = NULL, body = NULL, as_req = FALSE) {
...
#'   
#' @keywords internal
oa_prepare_ids_new <- function(ids) {
...
#'   
#' @keywords internal
oa_parse_year_filter_new <- function(year_filter) {
...
#'   
#' @keywords internal
oa_process_response_new <- function(data) {
  if (nrow(data) == 0) {
    return(data.frame())
  }
  ...
  # Extract abstracts
  abstract_vec <- oa_extract_abstract_new(data[["abstract_inverted_index"]])
...
#'   
#' @keywords internal
oa_extract_abstract_new <- function(abstract_inverted_index) {
